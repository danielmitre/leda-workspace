...
public class AVLTreeImpl<T extends Comparable<T>> extends BSTImpl<T> implements
	...
	protected void rebalance(BSTNode<T> node) {
		...
		System.out.println("o balanco de "+node+" eh "+balanco);
		if (balanco >= 2){	//desbalanceado para esquerda
			filho = (BSTNode<T>) node.getLeft();
			if (calculateBalance(filho) <= -1){	//pendendo para direita
				System.out.println("rotacionando "+filho.getData()+" para esquerda");
				rotacionaEsquerda(filho);
			}
			System.out.println("rotacionando "+node.getData()+" para direita");
			rotacionaDireita(node);
		} else if (balanco <= -2){
			filho = (BSTNode<T>) node.getRight();
			if (calculateBalance(filho) >= 1){
				System.out.println("rotacionando "+filho.getData()+" para direita");
				rotacionaDireita(filho);
			}
			System.out.println("rotacionando "+node.getData()+" para esquerda");
